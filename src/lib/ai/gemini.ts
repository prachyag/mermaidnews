...
/**
 * คำสั่งเรื่องความยาว/เนื้อหาของแคปชัน — **ตัดสินจากวัตถุดิบที่มีจริง ไม่ใช่การตั้งค่า**
 *
 * มีเนื้อข่าวเต็มจากเว็บ = มีของให้เล่า จึงสั่งเขียนยาวได้
 * มีแค่พาดหัว + เนื้อย่อ RSS = สั่งให้เขียนสั้น เกริ่นให้กดลิงก์
 *
 * เดิมเป็นสวิตช์ต่อหัวข้อ (captionIncludeSummary) ซึ่งสั่ง "เขียนยาวไม่จำกัด" ได้
 * ทั้งที่ต้นทางมีแค่สองบรรทัด = เชิญชวนให้ AI แต่งเติมเพื่อเติมความยาวโดยตรง
 * ผูกกับ content แทน ทำให้ไม่มีทางขอความยาวเกินกว่าข้อมูลที่มีอยู่จริงได้เลย
 *
 * แยกเป็นฟังก์ชันเพราะใช้ทั้งแบบเดี่ยวและแบบชุด ต้องพูดตรงกันเป๊ะ
 */
function captionInstruction(hasFullContent: boolean): string {
...
4. ถ้า relevant เป็น true ให้ตอบเพิ่ม:
   ...
   - caption: ${captionInstruction(false)} — ห้ามใส่ลิงก์และห้ามใส่แฮชแท็กในแคปชัน (ระบบจะแนบให้เอง)
...
3. ถ้า relevant เป็น true ให้ตอบเพิ่ม:
   - summary: สรุปข่าวเป็นภาษาไทย 1–2 ประโยค
   - caption: ${captionInstruction(Boolean(input.content))} ${input.captionStyle ? `สไตล์: ${input.captionStyle}` : "โทนเป็นกันเอง อ่านง่าย"} — ห้ามใส่ลิงก์และห้ามใส่แฮชแท็กในแคปชัน (ระบบจะแนบให้เอง)
   - hashtags: แฮชแท็กภาษาไทย/อังกฤษ 3–5 อัน (ขึ้นต้นด้วย #)
